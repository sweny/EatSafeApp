...
exports.register = function(fname,lname,callback) {
	if(!(email.indexOf("@")<1 || email.lastIndexOf(".")< email.indexOf("@")+2 || email.lastIndexOf(".")+2 >= email.length)) {
		...
		connection.beginTransaction(function(err) {
			...
			connection.query(sql, function(err, result) {
				console.log("err = "+err);
				if (result[0].count > 0) {					
					var values = {firstname: fname, lastname: lname};
					connection.query('Update INTO users SET ?', values, function(err,result){
						if (err) {
							console.log(err);
